...
namespace WinPE_Client.Services
{
    ...
    public static class EfiSysGenerator
    {
        ...
        {
            int fatOffset = (cluster / 2) * 3;
            if ((cluster & 1) == 0)
            {
                fat[fatOffset] = (byte)(value & 0xFF);
                fat[fatOffset + 1] = (byte)((fat[fatOffset + 1] & 0xF0) | ((value >> 8) & 0x0F));
            }
            else
            {
                fat[fatOffset] = (byte)(((value & 0x0F) << 4) | (fat[fatOffset] & 0x0F));
                fat[fatOffset + 1] = (byte)((value >> 4) & 0xFF);
            }
        }
