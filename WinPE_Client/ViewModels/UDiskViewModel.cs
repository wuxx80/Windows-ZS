...
namespace WinPE_Client.ViewModels
{
    ...
    public class UDiskViewModel : INotifyPropertyChanged
    {
        ...
        private void BuildSummary()
        {
            ...
            SummaryItems.Add(new UdiskSummaryItem { Icon = "\uD83D\uDCE6", Title = "PE 源", Content = IsLocalSource ? PeFilePath : (SelectedPeVersion?.DisplayText ?? "") });
            ...
            SummaryItems.Add(new UdiskSummaryItem { Icon = "\uD83D\uDDA5\uFE0F", Title = "附加", Content = (IncludeClient ? "写入装机助手客户端" : "不写入客户端") + (ApplyCustomize ? " · 应用 PE 定制" : "") + (IncludeTools ? " · 写入内置工具" : "") });
        }
