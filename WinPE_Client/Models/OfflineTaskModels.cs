...
{
    /// <summary>
    /// 离线无人值守任务（zs_task.json）—— 方案C 双保险的离线注入载体。
    /// 同一份格式由两种来源写入：
    ///  · 方案A：U盘/ISO 制作时选择「写入装机镜像」注入（source = usb）
    ///  · 方案B：Windows 下单时预下载镜像到数据盘并注入（source = windows）
    ...
    public class OfflineImageInfo
    {
        ...
        /// <summary>镜像路径：绝对路径，或相对 zs_task.json 所在目录（如 ZS_Images\win11.wim）</summary>
