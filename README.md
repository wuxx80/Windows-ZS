...
> **R7-E 子轮状态（2026-09-02）：一键装机界面精简为 3 步向导（选择镜像→目标磁盘与选项→执行进度），ViewModel 同步更新；修复镜像管理 5 项问题（字段名错误/删除无效/状态显示异常/edit 字段解析/软删除视效）；PE 清理逻辑默认开启（FirstBootCleanup=true）；后台菜单移除客户端管理 3 项无功能入口。三端编译 0 错 0 警。**
...
### 后台管理（现行菜单 14 项）
控制台 / 镜像列表 / 镜像源 / 镜像标签 / 无人值守模板 / 软件管理 / 软件分类 / 软件模板 / 驱动 / 脚本 / PE 版本 / PE 定制 / 用户管理 / 系统设置 / 操作日志。

> 已从菜单移除（底层保留作过渡期复用，Phase 3 评估是否永久删除）：PXE 配置、网络部署、客户端管理（3 项）、任务管理（2 项）、客户/工单、通知/定时任务/报表统计/Webhook 日志/回收站。
